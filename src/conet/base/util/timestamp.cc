...
Timestamp Timestamp::addTimeSecond(Timestamp timestamp, double second) {
...
Timestamp Timestamp::addTimeMilliSecond(Timestamp timestamp, double millisecond) {
    int64_t microseconds = millisecond * 1000;
    return Timestamp(timestamp.m_systemTimePoint + std::chrono::microseconds(static_cast<long long>(microseconds)));
}

Timestamp Timestamp::addTimeMicroSecond(Timestamp timestamp, double microsecond) {
